/*
	...
	Lint the JavaScript files in <script> tags in examples and docs
...
class ESLintScriptTags extends BaseLinter {

	async generateListOfFiles() {

		try {

			this.files = glob.sync( '{examples,docs/**}/*.html', { absolute: true, cwd: this.basePath } )
	...
	async worker( ) {
		...
		const cli = new CLIEngine( {
			baseConfig: {
				extends: [ 'mdcs', 'eslint-config-mdcs' ]
			},
			parser: 'espree',
			plugins: [ 'html' ],
			useEslintrc: false,
			// globals: [ 'THREE', 'WEBGL', 'Stats', 'dat', 'controls' ],
			rules: { "no-undef": 0 },
			ignorePattern: [ '*.js' ]
		} );
...
if ( require.main === module ) {
	...
	( async() => {
		...
		// analyze
		const linter = new ESLintScriptTags( basePath, outputStream );
...
module.exports = ESLintScriptTags;
