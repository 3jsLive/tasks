...
// *** modified jsdoc and code formatting
/**
 ...
 * @returns {Promise<{stdout: string, stderr: string, code: number}>} - Resolves with the command results from `stdout` and `code`
 */
function execAsync( cmd, opts = {} ) {
	...
	return new Promise( function ( resolve, reject ) {
	  ...
	  shell.exec( cmd, opts, function ( code, stdout, stderr ) {
			...
			if ( code != 0 ) return reject( { stdout: stdout, code: code, stderr: stderr } );
			return resolve( { stdout: stdout, code: code, stderr: stderr } );
