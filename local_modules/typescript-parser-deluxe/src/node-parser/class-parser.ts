    ...
    return target.reduce(
        (all: TshParameter[], cur: ParameterDeclaration) => {
            ...
            if (cur.type && (<any>cur.type).members) {
                ...
                params.push(new TshParameter(
                    <string>(cur.name as Identifier).escapedText, getNodeType(cur, cur.type), !!cur.questionToken, cur.getStart(), cur.getEnd(),
                ));
            }
            return params;
        },
    ...
    node.parameters.forEach((o) => {
        if (isIdentifier(o.name)) {
            ctor.parameters.push(
                new TshParameter(
                    (o.name as Identifier).text, getNodeType(o, o.type), !!o.questionToken, o.getStart(), o.getEnd(),
                ),
            );
            if (!o.modifiers) {
                return;
            ...
            ctor.parameters = ctor.parameters.concat(<TshParameter[]>elements.map((bind: any) => {
                if (isIdentifier(bind.name)) {
                    return new TshParameter(
                        (bind.name as Identifier).text, undefined, !!bind.questionToken, bind.getStart(), bind.getEnd(),
                    );
                }
            }).filter(Boolean));
        }
