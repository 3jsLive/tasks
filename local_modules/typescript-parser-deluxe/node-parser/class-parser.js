...
function parseCtorParams(parent, ctor, node) {
    ...
    node.parameters.forEach((o) => {
        if (TypescriptGuards_1.isIdentifier(o.name)) {
            ctor.parameters.push(new ParameterDeclaration_1.ParameterDeclaration(o.name.text, parse_utilities_1.getNodeType(o, o.type), !!o.questionToken, o.getStart(), o.getEnd()));
            if (!o.modifiers) {
                return;
            }
            ...
            ctor.parameters = ctor.parameters.concat(elements.map((bind) => {
                if (TypescriptGuards_1.isIdentifier(bind.name)) {
                    return new ParameterDeclaration_1.ParameterDeclaration(bind.name.text, undefined, !!bind.questionToken, bind.getStart(), bind.getEnd());
                }
            }).filter(Boolean));
        }
    });
