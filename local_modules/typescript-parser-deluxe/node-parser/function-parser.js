...
function parseTypeArguments(node) {
    ...
    return target.reduce((all, cur) => {
        ...
        if (cur.type && cur.type.members) {
            params.push(new ParameterDeclaration_1.ParameterDeclaration(cur.name.escapedText, parseTypeArguments(cur.type.members), !!cur.questionToken, cur.getStart(), cur.getEnd()));
        }
        else {
            params.push(new ParameterDeclaration_1.ParameterDeclaration(cur.name.escapedText, parse_utilities_1.getNodeType(cur, cur.type), !!cur.questionToken, cur.getStart(), cur.getEnd()));
        }
        return params;
    }, []);
}
...
function parseMethodParams(node) {
    return node.parameters.reduce((all, cur) => {
        ...
        if (TypescriptGuards_1.isIdentifier(cur.name)) {
            params.push(new ParameterDeclaration_1.ParameterDeclaration(cur.name.text, parse_utilities_1.getNodeType(cur, cur.type), !!cur.questionToken, cur.getStart(), cur.getEnd()));
        }
            ...
            boundParam.parameters = elements.map((bindingElement, index) => new ParameterDeclaration_1.ParameterDeclaration(bindingElement.name.getText(), types[index], false, // ***
            bindingElement.getStart(), bindingElement.getEnd()));
            ...
            boundParam.parameters = elements.map((bindingElement, index) => new ParameterDeclaration_1.ParameterDeclaration(bindingElement.getText(), types[index], false, // ***
            bindingElement.getStart(), bindingElement.getEnd()));
            params.push(boundParam);
        }
        return params;
