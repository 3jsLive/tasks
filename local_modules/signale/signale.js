...
class Signale {
  ...
  _logger(type, ...messageObj) {
	...
	// ***
	// this is all horrid, but it works

	const whitelist = ( process.env[ 'SIGNALE' ] ) ? process.env[ 'SIGNALE' ].split( ',' ) : [];

	// negative
	if ( whitelist.includes( `-${this.scopeName}` ) ||				// whitelist includes -scope?
	whitelist.includes( `-${this.scopeName}:${type}` ) ||			// ..or -scope:label?
	whitelist.includes( `-${this.scopeName}:*` ) ||					// ..or -scope:*?
	whitelist.includes( `-*:${type}` ) )							// ..or -*:label?
		return;														// then don't show the message

	// a lone wildcard in there? display all
	if ( whitelist.includes( '*' ) === false ) {

		// positive
		if ( whitelist.includes( `${this.scopeName}` ) === false &&		// whitelist doesn't include the current scope on its own?
		whitelist.includes( `${this.scopeName}:${type}` ) === false &&	// ..or scope:label?
		whitelist.includes( `${this.scopeName}:*` ) === false &&		// ..or scope:*?
		whitelist.includes( `*:${type}` ) === false )					// ..or *:label?
			return;														// then don't show it
